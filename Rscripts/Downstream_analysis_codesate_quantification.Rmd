---
title: "Downstream analysis followig the image analysis to quntify Dvl2_mEos codensates"
author:  
- "Christian Scheeder, Antonia Schubert"
...
thresholding (FITC channel) and counted. Condensate counts and cell counts (number of segmeted cells) were saved for each image.
An exemplary image analysis can be found with the script
XXX.R in this repository.
Four different cell lines are compared: 
...
differeces betwee the cell lines were compared using a boxplot and statistical 
...
```{r}
results_path <- here(
  "data/condesate_quantification/results_image_analysis/data_frames")
...
ggsave(here("figures/Figure4","boxplot_figure4_raw.pdf"),
       plot=boxplot,
       width = 6,
       height = 4)
```
